...
const AddQuestion = () => {
  ...
  const handleMultipleCorrectToggle = () => {
    setFormData(prev => ({
      ...prev,
      isMultipleCorrect: !prev.isMultipleCorrect,
      correctAnswers: []
    }));
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-800 mb-8 text-center">Create New MCQ</h1>
            ...
            <div className="space-y-6">
              ...
              <div className="border-b border-gray-200 pb-4">
                ...
                <div className="flex items-center space-x-4 text-sm text-gray-600">
                  <span>Topic: {formData.topic || 'Not specified'}</span>
                  {formData.subtopic && (
                    <>
                      <span>•</span>
                      <span>Subtopic: {formData.subtopic}</span>
                    </>
                  )}
                </div>
              </div>
  ...
  {formData.questionText}
    ...
    <p className="text-gray-500">Your question text will appear here...</p>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-gray-500">
                    {formData.isMultipleCorrect ? 'Multiple correct answers allowed' : 'Single correct answer'}
                  </span>
                </div>
                {formData.options.map((option, index) => (
                  <div
                  ...
                  >
                    ...
                    <span className="text-gray-800 flex-1">
                      {option || `Option ${index + 1} placeholder`}
                    </span>
                    {formData.correctAnswers.includes(index) && (
                      <span className="text-[#4CA466] text-sm font-medium">✓ Correct</span>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                <h4 className="font-medium text-gray-800 mb-3">Question Settings</h4>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="font-medium text-gray-700">Marks: </span>
                    <span className="text-gray-600">{formData.marks || '0'}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">Negative Marks: </span>
                    <span className="text-gray-600">{formData.negativeMarks || '0'}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">Time Limit: </span>
                    <span className="text-gray-600">
                      {formData.timeLimit ? `${formData.timeLimit} ${formData.timeUnit}` : 'Not set'}
                    </span>
                  </div>
                  <div>
              ...
              {/* Question Summary */}
              <div className="bg-[#4CA466]/5 rounded-lg p-4 border border-[#4CA466]/20">
                <h4 className="font-medium text-[#4CA466] mb-2">Question Summary</h4>
                <div className="text-sm text-gray-700 space-y-1">
                  <p>Total Options: {formData.options.length}</p>
                  <p>Correct Answers: {formData.correctAnswers.length}</p>
                  <p>Question Type: {formData.isMultipleCorrect ? 'Multiple Choice (Multiple Correct)' : 'Multiple Choice (Single Correct)'}</p>
                </div>
              </div>
            </div>
          </div>
