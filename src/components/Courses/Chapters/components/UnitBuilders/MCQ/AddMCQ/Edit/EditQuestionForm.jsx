...
export default function EditQuestionForm({ formData, setFormData, setSaveRef, setSaving }) {
  ...
  const validateForm = () => {
    ...
    // ensure all uploaded images have a non-empty label
    const allImgs = [
      ...(formData.questionImages || []),
      ...(formData.explanationImages || []),
      ...((formData.options || []).flatMap(o => (o.images || [])))
    ];
    if (allImgs.some(img => !img || !String(img.label || '').trim())) {
      return showError('All uploaded images must have a label'), false;
    }
  ...
  const FileUploader = ({ purpose, optIndex = null }) => (
    <div className="mt-2">
      <label className="block text-sm font-medium text-gray-700 mb-2">
        {purpose === 'question' ? 'Question Images' : purpose === 'explanation' ? 'Explanation Images' : 'Option Images'}
      </label>
      ...
      <div className="flex gap-2 mt-2 flex-wrap">
        {purpose === 'question' && (formData.questionImages || []).map((img, i) => (
          <div key={img.image_id} className="border rounded p-2 flex flex-col items-start w-36">
            <img src={img.url} alt={img.alt_text || img.label} className="w-32 h-20 object-cover rounded" />
            <div className="w-full mt-2">
              <input
                ref={el => { if (img.image_id) imageInputRefs.current[img.image_id] = el; }}
                type="text"
                value={img.label || ''}
            ...
            <div className="flex items-center justify-between w-full mt-1">
              ...
              <button onClick={() => removeImage('question', i)} className="text-red-500 text-xs">Remove</button>
            </div>
          </div>
        ))}
        ...
        {purpose === 'explanation' && (formData.explanationImages || []).map((img, i) => (
          <div key={img.image_id} className="border rounded p-2 flex flex-col items-start w-36">
            ...
            <div className="flex items-center justify-between w-full mt-1">
              <div className="text-xs text-gray-500">{formatFileSize(img.metadata?.size || 0)}</div>
              <button onClick={() => removeImage('explanation', i)} className="text-red-500 text-xs">Remove</button>
            </div>
          </div>
        ))}

        {purpose === 'option' && optIndex !== null && ((formData.options[optIndex] && formData.options[optIndex].images) || []).map((img, i) => (
          <div key={img.image_id} className="border rounded p-2 flex flex-col items-start w-36">
            <img src={img.url} alt={img.alt_text || img.label} className="w-32 h-20 object-cover rounded" />
            <div className="w-full mt-2">
              <input
                ...
                type="text"
                value={img.label || ''}
                onChange={(e) => updateImageLabel('option', i, e.target.value, optIndex)}
                placeholder="Enter label..."
                className="text-xs px-2 py-1 border rounded w-full"
              />
            </div>
            <div className="flex items-center justify-between w-full mt-1">
              <div className="text-xs text-gray-500">{formatFileSize(img.metadata?.size || 0)}</div>
              <button onClick={() => removeImage('option', i, optIndex)} className="text-red-500 text-xs">Remove</button>
            </div>
          </div>
        ))}
      </div>
