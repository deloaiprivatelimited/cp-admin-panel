...
import { getQuestionForm,saveQuestionForm } from "../coding"; 
import { showError,showSuccess } from "../../../../../../../utils/toast";
...
const mdProps = {
  ...
  components: {
    ...
    code: ({ inline, className, children, ...props }) => {
      if (inline) {
        return <code className="bg-gray-100 px-1 rounded text-xs font-mono">{children}</code>;
      }
      // block code: render in terminal-like container
      return (
        <div className="rounded-md overflow-hidden border border-gray-200">
          <div className="flex items-center gap-2 px-3 py-1 bg-gray-900 text-white">
            <div className="flex items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-red-500" />
              <span className="h-2 w-2 rounded-full bg-yellow-400" />
              <span className="h-2 w-2 rounded-full bg-green-400" />
            </div>
            <div className="text-xs text-gray-200 ml-2">{(className || "").replace("language-", "")}</div>
          </div>
          <pre className="bg-gray-900 text-gray-100 font-mono text-sm p-3 overflow-x-auto">
            ...
            <div className="space-y-4">
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-900">
                  <MarkdownRenderer text={formData.title || "Question Title"}  />
