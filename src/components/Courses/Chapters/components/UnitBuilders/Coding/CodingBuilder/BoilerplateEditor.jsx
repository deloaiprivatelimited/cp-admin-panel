...
import { getPredefinedBoilerplates,upsertPredefinedBoilerplates } from '../coding';
import { showError } from '../../../../../../../utils/toast';
// import { getPredefinedBoilerplates, upsertPredefinedBoilerplates } from '../services/codingQuestions';
// import { showError } from '../../../../utils/toast';
function BoilerplateEditor({ questionId }) {
...
int main() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      setFetchError(null);
      try {
        const resp = await getPredefinedBoilerplates(questionId);
        // backend returns { id, predefined_boilerplates: { lang: code } }
        const remote = (resp && resp.predefined_boilerplates) || {};
...
languages.forEach(l => {
        ...
        showError('Failed to fetch boilerplates', err);
        if (mounted) {
  ...
  const saveCurrentLanguage = useCallback(async () => {
    ...
    try {
      ...
      await upsertPredefinedBoilerplates(questionId, { language: lang, code });
      setSaveStatus('saved');
  ...
  const saveAllLanguages = useCallback(async () => {
    ...
    try {
      await upsertPredefinedBoilerplates(questionId, { predefined_boilerplates: boilerplates });
      setSaveStatus('saved');
      setLastSaved(new Date());
            languages.forEach(l => setLangStatus(l.value, { saved: true, saving: false, error: null }));

    } catch (err) {
      showError('Save all failed', err);
      setSaveStatus('error');
    ...
    <div className="h-screen bg-gray-50 flex flex-col">
      ...
      <div className="p-6 bg-white shadow-sm border-b">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Boilerplate Editor</h1>
